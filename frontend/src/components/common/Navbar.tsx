...
export const Navbar: React.FC = () => {
  ...
  const getRoleBadgeTitle = () => {
    switch (role) {
      case 'MINISTER': return "Hon'ble Minister Executive";
      case 'MINISTRY': return "National MoSPI Ops";
      case 'STATE': return `State Nodal Authority (${user?.state || 'UP'})`;
      case 'DISTRICT': return `District Collector (${user?.district || 'Varanasi'})`;
      default: return 'Authority Account';
    }
  };
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden sm:block text-left text-xs">
            ...
            <p className="text-[10px] text-blue-700 font-semibold mt-0.5">{getRoleBadgeTitle()}</p>
          </div>
        </div>
