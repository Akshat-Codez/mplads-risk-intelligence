...
export function getAuthorityScopeFilter(user) {
  ...
  // 1. National Scope: Ministry / Super Admin / Minister -> Nationwide access
  if (['MINISTRY', 'ADMIN', 'SUPER_ADMIN', 'MINISTER'].includes(role)) {
    return {};
  }

  // 2. State Authority Scope -> Restricted strictly to their assigned state
  if (role === 'STATE') {
